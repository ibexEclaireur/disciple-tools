...
/**
 * Disciple_Tools_REST_API
 *
 * @class Disciple_Tools_REST_API
 * @version	0.1
 * @since 0.1
...
class Disciple_Tools_REST_API {
    ...
    /**
     * Disciple_Tools_REST_API The single instance of Disciple_Tools_REST_API.
     * @var 	object
     * @access  private
     * @since 	0.1
     */
    ...
    /**
     * Main Disciple_Tools_REST_API Instance
     *
     * Ensures only one instance of Disciple_Tools_REST_API is loaded or can be loaded.
     *
     * @since 0.1
     * @static
     * @return Disciple_Tools_REST_API instance
     */
    ...
    public function find_by_address (WP_REST_Request $request){
        ...
        if (isset($params['address'])){
            $result = Disciple_Tools_Controller::get_tract_by_address($params['address']);
            if ($result["status"] == 'OK'){
                return $result["tract"];
            } else {
    ...
    public function get_tract_map (WP_REST_Request $request){
        $params = $request->get_params();
        if (isset($params['address'])){
            $result = Disciple_Tools_Controller::get_tract_map($params['address']);
            if ($result["status"] == 'OK'){
                return $result;
            } else {
    ...
    public function get_map_by_geoid (WP_REST_Request $request){
        ...
        if (isset($params['geoid'])){
            $result = Disciple_Tools_Controller::get_map_by_geoid ($params);
            if ($result["status"] == 'OK'){
                return $result;
            } else {
                return new WP_Error("map_status_error", $result["message"], array('status', 400));
